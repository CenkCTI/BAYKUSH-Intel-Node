...
async function run(): Promise<void> {
  ...
  try {
    ...
    const livePartial = await insertRevision({ revision: 1, basis: "LIVE_STREAM", coverage: "PARTIAL", availability: "PARTIAL", fingerprint: "a".repeat(64), supersedes: null, liveCoverage: "PARTIAL" });
    await client.query(
    ...
    const recovered = await insertRevision({ revision: 2, basis: "MRT_RECOVERY", coverage: "COMPLETE", availability: "AVAILABLE", fingerprint: "b".repeat(64), supersedes: livePartial, liveCoverage: "PARTIAL" });
    ...
    const latePartial = await insertRevision({ revision: 3, basis: "LIVE_STREAM", coverage: "PARTIAL", availability: "PARTIAL", fingerprint: "c".repeat(64), supersedes: recovered, liveCoverage: "PARTIAL" });
    ...
    const liveComplete = await insertRevision({ revision: 4, basis: "LIVE_STREAM", coverage: "COMPLETE", availability: "AVAILABLE", fingerprint: "d".repeat(64), supersedes: recovered, liveCoverage: "COMPLETE" });
