...
async function statuses(url:URL,response: ServerResponse, id: string): Promise<void> {
  ...
  sendEnvelope(response, 200, result.rows, id, { selectedRange:{from:from.toISOString(),to:toISOString()},note: "Operational health, freshness, selected-range coverage, availability and backfill state are independent." });
}
