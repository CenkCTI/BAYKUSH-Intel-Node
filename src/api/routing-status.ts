...
export async function routingStatus(response: ServerResponse, id: string): Promise<void> {
  ...
  const [heartbeatResult, sessionResult, recoveryResult, latestResult] = await Promise.all([
  ...
  sendEnvelope(response, 200, {
    ...
    latest: latest ? {
      bucketStart: latest.bucket_start.toISOString(),
      bucketEnd: latest.bucket_end.toISOString(),
      updateMessages: latest.update_messages,
      announcementPrefixEvents: latest.announcement_prefix_events,
      withdrawalPrefixEvents: latest.withdrawal_prefix_events,
      distinctPrefixesObserved: latest.distinct_prefixes_observed,
      distinctOriginAsnsObserved: latest.distinct_origin_asns_observed,
      rrcCount: latest.rrc_count,
      coverageStatus: latest.coverage_status,
      dataAvailability: latest.data_availability,
      acquisitionBasis: latest.acquisition_basis,
      acquisitionChannel: routingAcquisitionChannel(latest.acquisition_basis),
      liveCollectionCoverage: latest.live_collection_coverage,
      captureProfileKey: latest.capture_profile_key,
      captureProfileVersion: latest.capture_profile_version,
      captureProfileRrcCount: latest.capture_profile_rrc_count,
    } : null,
  }, id, {
    heartbeatStaleAfterMs: staleAfterMs,
    note: "Operational worker freshness, live collection coverage, current data availability, and acquisition basis are independent. MRT recovery never rewrites historical live collection coverage.",
  });
}
