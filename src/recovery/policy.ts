...
export const recoveryFailureCodes = [
  "ARCHIVE_NOT_READY","HTTP_NOT_FOUND","HTTP_RATE_LIMITED","HTTP_SERVER_ERROR","DOWNLOAD_TIMEOUT",
  "DOWNLOAD_SIZE_LIMIT","DOWNLOAD_TLS_ERROR","DOWNLOAD_REDIRECT_REJECTED","DISK_WATERMARK",
  "ARTIFACT_HASH_CHANGED","GZIP_INVALID","DECODER_TIMEOUT","DECODER_MEMORY_LIMIT","DECODER_EXIT_NONZERO",
  "DECODER_OUTPUT_INVALID","DECODER_OUTPUT_LIMIT","DECODER_CORRUPT_RECORD","POPULATION_MISMATCH",
  "TIMESTAMP_ANOMALY","PROJECTION_CONFLICT","DATABASE_FAILURE",
] as const;
export type RecoveryFailureCode = typeof recoveryFailureCodes[number];

const retryable = new Set<RecoveryFailureCode>([
  "ARCHIVE_NOT_READY","HTTP_NOT_FOUND","HTTP_RATE_LIMITED","HTTP_SERVER_ERROR","DOWNLOAD_TIMEOUT",
  "DOWNLOAD_TLS_ERROR","DISK_WATERMARK","DATABASE_FAILURE",
]);
export function isRetryableRecoveryFailure(code: RecoveryFailureCode): boolean { return retryable.has(code); }

const automaticReasons = new Set(["BACKPRESSURE_LIMIT","PROVIDER_DISCONNECT","DB_UNAVAILABLE","UNEXPECTED_RESTART","NETWORK_DISCONNECT"]);
export function isAutomaticRecoveryReason(reason: string): boolean { return automaticReasons.has(reason); }
...
export function assertRecoveryRange(from: string, to: string, automatic: boolean): { fromMs: number; toMs: number } {
  ...
  if (!Number.isFinite(fromMs) || !Number.isFinite(toMs) || toMs <= fromMs) throw new Error("Recovery range must be valid RFC3339 with to > from");
  const limit = (automatic ? recoveryPolicyV1.automaticGapMaxSeconds : recoveryPolicyV1.manualRequestMaxSeconds) * 1000;
