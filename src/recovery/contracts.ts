...
export function definitiveOriginAsns(segments: MrtDecoderRecord["asPathSegments"]): number[] {
  const last = segments.at(-1); if (!last || last.kind !== "AS_SEQUENCE") return [];
  const value = last.asns.at(-1); return value === undefined ? [] : [value];
}
...
export function decoderRecordToRoutingObservation(input: {
  ...
  const identity = {
    upstream: "RIPE_RIS", artifactSha256: input.artifactSha256, recordIndex: input.record.recordIndex,
    sourceObservedAt: input.record.sourceObservedAt, peerAsn: input.record.peerAsn, peerIp: input.record.peerIp,
    announcedPrefixes, withdrawnPrefixes, asPathSegments: input.record.asPathSegments,
  };
  return {
