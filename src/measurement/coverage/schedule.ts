...
function classifyOpportunity(run: CoverageRunInput | undefined): Omit<ExpectedOpportunity, "expectedAt"> {
  ...
  if (!liveSatisfyingRun(run)) return { status: "MISSING", matchedRunId: run.id, reasonCode: "NON_LIVE_RUN_DOES_NOT_PROVE_COVERAGE" };
  if (run.state === "SUCCEEDED") return { status: "SATISFIED", matchedRunId: run.id, reasonCode: "SCHEDULED_RUN_SUCCEEDED" };
  if (run.state === "PARTIAL") return { status: "PARTIAL", matchedRunId: run.id, reasonCode: "SCHEDULED_RUN_PARTIAL" };
  if (["FAILED", "CANCELLED"].includes(run.state)) return { status: "FAILED", matchedRunId: run.id, reasonCode: `SCHEDULED_RUN_${run.state}` };
...
export function evaluateScheduledCoverage(input: {
  ...
  if (!Number.isInteger(maxOpportunities) || maxOpportunities < 1 || maxOpportunities > 100_000) throw new Error("Invalid coverage opportunity bound");
  ...
  const fingerprintBase = { schedule: input.schedule, runs: input.runs, bucketStart: input.bucketStart, bucketEnd: input.bucketEnd };

  if (input.schedule.originStatus === "UNKNOWN") {
    return { expectationStatus: "UNKNOWN", coverageStatus: "NO_COVERAGE", evaluationState: evaluatedAt < bucketEnd + graceMs ? "PROVISIONAL" : "FINAL", expectedOpportunityCount: 0, satisfiedOpportunityCount: 0, partialOpportunityCount: 0, failedOpportunityCount: 0, missingOpportunityCount: 0, opportunities: [], reasonCodes: ["SCHEDULE_EXPECTATION_UNKNOWN"], inputFingerprint: sha256(fingerprintBase) };
  }
  if (!input.schedule.enabled || input.schedule.pollIntervalSeconds === null || activeEnd <= activeStart) {
    return { expectationStatus: "NOT_EXPECTED", coverageStatus: "NO_COVERAGE", evaluationState: "FINAL", expectedOpportunityCount: 0, satisfiedOpportunityCount: 0, partialOpportunityCount: 0, failedOpportunityCount: 0, missingOpportunityCount: 0, opportunities: [], reasonCodes: [input.schedule.enabled ? "NO_SCHEDULED_CADENCE" : "SOURCE_NOT_EXPECTED"], inputFingerprint: sha256(fingerprintBase) };
  }

  const intervalMs = input.schedule.pollIntervalSeconds * 1_000;
  const runAnchors = input.runs.filter(naturalAnchorRun).map((run) => milliseconds(run.scheduledFor as string)).filter((value) => value >= scheduleStart && value < scheduleEnd);
  ...
  for (let index = 0; index < anchors.length; index += 1) {
    ...
    const segmentEnd = Math.min(activeEnd, nextAnchor ?? activeEnd);
    if (segmentEnd <= activeStart || anchor >= activeEnd) continue;
    const firstStep = Math.max(0, Math.ceil((activeStart - anchor) / intervalMs));
    for (let expected = anchor + firstStep * intervalMs; expected < segmentEnd; expected += intervalMs) {
      if (expected + graceMs > evaluatedAt) continue;
      if (nextAnchor !== null && Math.abs(nextAnchor - expected) <= graceMs) continue;
      expectedTimes.push(expected);
      if (expectedTimes.length > maxOpportunities) throw new Error("Coverage opportunity bound exceeded");
    }
  }
  for (const anchor of runAnchors) if (anchor >= activeStart && anchor < activeEnd && anchor + graceMs <= evaluatedAt) expectedTimes.push(anchor);
  ...
  return { expectationStatus: "EXPECTED", coverageStatus, evaluationState, expectedOpportunityCount: opportunities.length, satisfiedOpportunityCount: satisfied, partialOpportunityCount: partial, failedOpportunityCount: failed, missingOpportunityCount: missing, opportunities, reasonCodes, inputFingerprint: sha256({ ...fingerprintBase, evaluatedAt: iso(Math.min(evaluatedAt, bucketEnd + graceMs)), opportunities }) };
}
